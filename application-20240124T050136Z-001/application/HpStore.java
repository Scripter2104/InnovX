...
public class HpStore extends DisplayPage implements Initializable,Runnable,HpProducts
{
	@FXML
	VBox mainVBox;
	    ...
	    for (int i = 0; i < 25; i++) 
	    {
	        ...
	        if (currentHBox != null)
	        {
	            ...
	            {   	
	                ...
	                {
	                    displayHpLaptop(currentIndex,tempBox);
	                }
	            });
	        }
	    }
	...
	protected void displayHpLaptop(int indexOfHBox,HBox tempBox) 
	{
		index = indexOfHBox;
		mrpOfLaptop = mrpOfHp[index];
	    try
	    {
	    	tempForCart = tempBox;
	        Stage hpStage = new Stage(); 
	        Parent hpRoot = FXMLLoader.load(getClass().getResource("HpLaptop.fxml"));
